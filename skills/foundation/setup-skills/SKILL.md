...
The defining constraint: this skill only records decisions, it never changes project tooling. It writes its own config, and with a yes it seeds vocabulary and installs an agent instructions block; it does not install packages, edit build config, or refactor.
...
- **Per repo, opt in**: `CONTEXT.md` and the verification block in `AGENTS.md` or `CLAUDE.md` are the only files outside `.jon-skills/` this skill writes, and only after an explicit yes.
...
- Write the verification block only between its markers, and only after a yes. On a repeat run, replace what is between the markers rather than appending a second copy, and if that text has been edited, show the difference and ask.
